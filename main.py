    ...
    for url in getting_url():
        ...
        for link in next(links):
            ...
            if checking_build(build.address) is None:
                sending_build(
                    new=build.new_building_name, addr=build.address, stract=build.structure,
                    off=build.offical_builder, year=build.year_of_construction, floor=build.floors_in_the_hourse,
                    pas=build.passenger_bodice, serv=build.service_lift, in_=build.in_home,
                    pemo=build.pemolition_planned, type=build.type_of_bilding, yard=build.yard,
                    par=build.participation_type, deadline=build.deadline,
                    parc=build.parking
                    )
